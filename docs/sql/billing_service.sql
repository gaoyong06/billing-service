...
CREATE TABLE IF NOT EXISTS `billing_record` (
    ...
    INDEX `idx_user_id_date` (`user_id`, `created_at`) COMMENT '用户消费记录索引'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='消费流水表';

-- Table: recharge_order
CREATE TABLE IF NOT EXISTS `recharge_order` (
    `order_id` VARCHAR(64) NOT NULL COMMENT '订单号（billing-service生成，格式：recharge_{user_id}_{timestamp}，作为主键，传给payment-service作为业务订单号order_id）',
    `user_id` VARCHAR(36) NOT NULL COMMENT '用户ID',
