...
func (r *rechargeOrderRepo) GetRechargeOrderByPaymentID(ctx context.Context, paymentID string) (*biz.RechargeOrder, error) {
	var m model.RechargeOrder
	if err := r.data.db.WithContext(ctx).Where("payment_id = ?", paymentID).First(&m).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		return nil, err
...
func (r *rechargeOrderRepo) UpdateRechargeOrderStatus(ctx context.Context, orderID, paymentID, status string) error {
...
func (r *rechargeOrderRepo) RechargeWithIdempotency(ctx context.Context, orderID, paymentID string, amount float64) error {
